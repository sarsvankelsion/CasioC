...
fn build_map() -> HashMap<char, Vec<u8>> {
    let chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzÁáÀàẢảÃãẠạĂăẮắẰằẲẳẴẵẶặÂâẤấẦầẨẩẪẫẬậÉéÈèẺẻẼẽẸẹÊêẾếỀềỂểỄễỆệÍíÌìỈỉĨĩỊịÓóÒòỎỏÕõỌọÔôỐốỒồỔổỖỗỘộƠơỚớỜờỞởỠỡỢợÚúÙùỦủŨũỤụƯưỨứỪừỬửỮữỰựÝýỲỳỶỷỸỹỴỵĐđ~@_&-+()/*':!?|√÷×^°{}[]%.,";
    let hexes = [
    ...
    for (ch, hx) in chars.chars().zip(hexes.iter()) {
        let bytes = if hx.len() == 2 {
            vec![u8::from_str_radix(hx, 16).unwrap()]
        } else {
            vec![u8::from_str_radix(&hx[0..2], 16).unwrap(), u8::from_str_radix(&hx[2..4], 16).unwrap()]
        };
        m.insert(ch, bytes);
    }
    // space and "~" both -> 0x20 (libcompiler does text.replace('~',' '))
    m.insert(' ', vec![0x20]);
    m.insert('~', vec![0x20]);
    m
}

pub fn encode_str(s: &str) -> Result<Vec<u8>, String> {
    ...
    for ch in s.chars() {
        if let Some(b) = map.get(&ch) {
            ...
            return Err(format!("char {ch:?} not in char_to_hex"));
        }
    }
    Ok(out)
}
...
mod tests {
    ...
    #[test]
    fn hello() {
        assert_eq!(encode_str("hi").unwrap(), vec![0x68, 0x69]);
    }
}
