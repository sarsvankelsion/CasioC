...
fn stdlib_candidates(name: &str) -> Vec<String> {
    ...
    let mut v = Vec::new();
    match k.as_str() {
        "screen_del" | "screen_clear" | "cls" => { v.push("buffer_clear".into()); v.push("buffer_clear.ca54".into()); v.push("buffer_clear.d654".into()); v.push("memzero".into()); }
        "screen_fill" => v.push("fill_screen".into()),
        "draw_line" | "line" => v.push("line_draw".into()),
        "draw_pixel" | "pixel" => v.push("pixel_draw".into()),
        "draw_rect" | "rect" => { v.push("line_draw".into()); }
        ...
        "print_at" | "draw_text" => v.push("line_print".into()),
        "print_small" => v.push("smallprint".into()),
        "print_num" | "print_dec" | "show_score" => v.push("hex_to_dec".into()),
        "print_hex" => v.push("hex_byte".into()),
        "render" | "flush" => { v.push("render".into()); v.push("render.ddd4".into()); v.push("render.ca54".into()); v.push("render_bitmap".into()); }
        "delay" | "sleep" => v.push("delay".into()),
        "get_key" | "key" => { v.push("getkey".into()); v.push("getkeycode".into()); v.push("getscancode".into()); v.push("cvt_key".into()); }
        "is_key_pressed" | "check_key" => v.push("check_any_key_pressed__".into()),
        "wait_key" | "pause" => v.push("diagnostic_wait_key".into()),
        "mem_copy" | "memcpy" => { v.push("memcpy".into()); v.push("smart_strcpy".into()); v.push("memmove".into()); }
        "mem_set" | "memset" => { v.push("memset".into()); v.push("memzero".into()); }
        "mem_zero" | "memzero" => v.push("memzero".into()),
        "mem_move" => v.push("memmove".into()),
        "double_buffer_flip" | "flip" => v.push("buf1_to_buf2".into()),
